...
static char initendflag;
static char WERflag; /* write/read/erase flag 0,1,2 */
...
static ssize_t powerloss_read(struct file *file, char __user *buf,
			size_t size, loff_t *ppos)
{
	if (size != 1)
		return -EINVAL;
	if (copy_to_user(buf, &WERflag, 1))
		return -EFAULT;
	return 1;

}
static long powerloss_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int err = 0;
	...
	case START_P_POWERLOSS:
		...
		pr_info("set powerloss mode: powerloss while programmimg.\n");
		break;

	case START_R_POWERLOSS:
		currPowerlossSt = START_R_POWERLOSS;
		pr_info("set powerloss mode: powerloss while reading.\n");
		break;

	case START_E_POWERLOSS:
		currPowerlossSt = START_E_POWERLOSS;
		pr_info("set powerloss mode: powerloss while erasing.\n");
		break;

	case START_NORMAL:
		currPowerlossSt = START_NORMAL;
		pr_info("set powerloss mode: powerloss while noraml status.\n");
		break;
	}
	return err;
}
...
static int powerloss_cdev_init(void)
{
	...
	if (!pcdev) {
		pr_err("%s:Couldn't powerloss cdev struct\n", devName);
		result = -ENOMEM;
		goto fail_malloc;
	}
	...
	device_create(powerloss_class, NULL,
			MKDEV(device_num, 0), NULL, "powerloss");
	initendflag = 1;
			...
			u8 R_W, u8 datalen)
{
	...
	if (host->mode == SDR_NAND_MODE) {
		...
		if (host_ready(mtd, WRITE_OP))
			dev_err(&pdev->dev, "%d Error:Host Waiting For Ready Timeout %d ms.\n",
...
static int nand_device_sr_ready(struct mtd_info *mtd)
{
	...
	dev_info(&host->pdev->dev, "Read SR is 0x%x.\n", sr);
	return ret;
}
...
static void set_clk(struct zed_nand_chip *zed_chip, int clk)
{
	...
	while ((readl(zed_chip->misc_regs + NAND_CLK) & B31) == 0)
		;
	dev_info(&zed_chip->pdev->dev, "Configure clock to %d MHz.\n\n", clk);
...
static void fpga_init(struct mtd_info *mtd)
{
	...
	PowerOn(mtd);
	udelay(100);
	dev_info(&pdev->dev, "The device version is %04x:%x.\n", readl(host->misc_regs + FPGA_VERSION),
...
static void __cmd_ctrl(struct mtd_info *mtd, int dat, unsigned int ctrl)
{
}
			...
			int column, int page_addr)
{
	...
	dev_dbg(&pdev->dev, "[1]------Send command 0x%x. address len %d.\n", command, addresslen);
	for (i = 0; i < addresslen; i++)
		dev_dbg(&pdev->dev, "Send address [%d]= 0x%x.\n", i, zed_address[i]);
	...
	case NAND_CMD_RESET:
		...
		if (1 != nand_device_sr_ready(mtd))
			dev_err(&pdev->dev, "%d Error:Device Waiting For Ready Timeout %d.\n",
			       __LINE__, READY_TIMEOUT_2000MS);
		return;
	...
	if (cmd_send == 1) {
		...
		dev_dbg(&pdev->dev, "[2]------Send command 0x%x.\n", command);
	}
			...
			int is_read)
{
	...
	if ((void *)buf >= high_memory) {
		dev_err(&host->pdev->dev, "buf address over high_memory 0x%p.\n", high_memory);
		goto err_buf;
	}

	dma_dev = host->dma_chan->device;

	phys_addr = dma_map_single(&host->pdev->dev, (void *)buf, len, dir);
	...
	if (host_ready(mtd, is_read)) {
		dev_err(&pdev->dev, "%d Error:Host Waiting For Ready Timeout %d.\n",
		       __LINE__, READY_TIMEOUT_2000MS);
		goto err_dma;
	}
...
static void zed_nand_read_buf(struct mtd_info *mtd, uint8_t *buf, int len)
{
	...
	if ((host->mode == DDR_NAND_MODE) && use_dma) {
		if (zed_nand_dma_op(mtd, &buf[0], len, READ_OP))
			dev_err(&pdev->dev, "%d DMA read error!\n", __LINE__);
	} else {
		if (host->mode == SDR_NAND_MODE) {
	...
	if (host->mode == SDR_NAND_MODE) {
		...
		if (use_dma && len > 2) {
			if (zed_nand_dma_op(mtd, &buf[0], len, WRITE_OP))
				dev_err(&pdev->dev, "%d DMA write error!\n", __LINE__);
		} else {
...
static uint8_t zed_nand_read_byte(struct mtd_info *mtd)
{
	...
	dev_dbg(&pdev->dev, "====>Read byte [0x%x],by %d mode \n", ret, host->mode);
...
static irqreturn_t zynq_irq_handler(int irq, void *dev_id)
{
	...
	dev_info(&host->pdev->dev, "interrupter is running.mode %d.\n", host->mode);
	return IRQ_HANDLED;
}

static int zed_nand_chip_check_interface(struct zed_nand_chip *host, int mode)
{
	struct platform_device *pdev = host->pdev;
	int ret = 0;
	...
	if (ret || (feature[0] & 0xF0) != check_mode) {
		dev_err(&pdev->dev, "Set nand mode error! Read mode is 0x%x.\n", feature[0]);
		return -1;
	}

	dev_info(&pdev->dev, "Set Nand mode %s successfully!\n",
			(mode == SDR_NAND_MODE ? "SDR" : "DDR"));

	return ret;
}

static int zed_nand_chip_init_timings(struct zed_nand_chip *host, int mode)
{
	struct platform_device *pdev = host->pdev;
	int ret = 0;
	int set_mode;
	...
	case DDR_NAND_MODE:
		...
		break;
	default:
		dev_err(&pdev->dev, "Interface Mode [%d] error.\n", mode);
		return -1;
	}

	/* [1] send SET FEATURE commond to NAND */
	feature[0] = set_mode;
	dev_info(&pdev->dev, "set mode value is 0x%x.\n", feature[0]);
...
static int zed_nand_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "misc_reg is start 0x%x.end 0x%x\n", misc_reg->start, misc_reg->end);
	dev_dbg(&pdev->dev, "sdr_reg is start 0x%x.end 0x%x\n", sdr_reg->start, sdr_reg->end);
	dev_dbg(&pdev->dev, "nvddr_reg is start 0x%x.end 0x%x\n", nvddr_reg->start, nvddr_reg->end);
	dev_dbg(&pdev->dev, "dma_reg is start 0x%x.end 0x%x\n", dma_reg->start, dma_reg->end);
	...
	dev_dbg(&pdev->dev, "remap misc_reg is start 0x%x.\n", (int)fnand->misc_regs);
	dev_dbg(&pdev->dev, "remap sdr_reg is start 0x%x.\n", (int)fnand->sdr_reg);
	dev_dbg(&pdev->dev, "remap nvddr_reg is start 0x%x.\n", (int)fnand->nvddr_reg);
	dev_dbg(&pdev->dev, "remap dma_regs is start 0x%x.\n", (int)fnand->dma_regs);
	...
	if (retval) {
		dev_err(&pdev->dev, "Scan NAND Device Failed!\n");
		if (retval > 0)
			retval = -ENXIO;
		goto out_free_data;
	}

	dev_info(&pdev->dev, "Using %u-bit/%u bytes BCH ECC\n", nand_chip->ecc_strength_ds,
	...
	*/
	dev_info(&pdev->dev, "nand_chip->ecc.size %d\n", nand_chip->ecc.size);
	...
	if (mode_transfer != fnand->mode) {
		...
		if (!retval) {
			...
			dev_info(&pdev->dev, "Send transfer-mode to %s command OK.\n",
		...
		if (retval) {
			dev_info(&pdev->dev, "Transfer-mode to %s mode failed.\n",
				(mode_transfer == DDR_NAND_MODE) ? "DDR" : "DDR2");
			dev_info(&pdev->dev, "Recover host mode to %s .\n",
				(mode_transfer == DDR_NAND_MODE) ? "SDR" : "DDR");
			change_host_mode(fnand, SDR_NAND_MODE);
			fnand->mode = SDR_NAND_MODE;
		} else
			dev_info(&pdev->dev, "Wonderful.Host and Device all transfer mode to %s mode.\n",
				(mode_transfer == DDR_NAND_MODE) ? "DDR" : "DDR2");
