...
#define ZED_NAND_DRIVER_VERSION "20181230"

struct zed_nand_controller {
...
static void SetVcc(struct mtd_info *mtd, u32 voltage)
{
	...
	struct zed_nand_controller *host = nc->priv;
...
static uint8_t zed_nand_read_byte(struct nand_chip *chip);
...
static int send_cmd(struct mtd_info *mtd, u8 cmd, u8 *addr, u8 addrlen,
                     u8 R_W, u8 datalen)
{
	register struct nand_chip *chip = mtd_to_nand(mtd);
	struct zed_nand_controller *host = chip->priv;
	int i;
	/* SDR mode */
...
static int SDR_PollingForHostReady(struct mtd_info *mtd)
{
	...
	register struct zed_nand_controller *host = chip->priv;
...
static void set_clk(struct zed_nand_controller *zed_chip, int clk)
{
...
static void set_sdr_clk(struct zed_nand_controller *zed_chip, int clk)
{
	...
	udelay(100);
	printk("Zed NAND controller Configure SDR mode clock to %d MHz.\n\n", clk);
...
static void fpga_init(struct mtd_info *mtd)
{
	...
	udelay(100);
	printk("The FPGA bit version is %04x:%x.\n", readl(host->misc_regs + FPGA_VERSION),
		readl(host->misc_regs + FPGA_SUB_VERSION));
	printk("The Host SW  version is %s.\n", ZED_NAND_DRIVER_VERSION);
}


static void set_ddr_DelayDqs(struct zed_nand_controller *host, u8 value)
{
...
static void change_host_mode(struct zed_nand_controller *host, int mode)
{
	u32 temp;
...
static void __cmd_ctrl(struct nand_chip *chip, int dat, unsigned int ctrl)
{
	return;
}
...
//			int page_addr);
static void __nand_command(struct nand_chip *chip, unsigned int command,
...
//	register struct nand_chip *chip = mtd->priv;
	...
	if (column != -1 || page_addr != -1) {
		...
		if (nanddev_target_size(&chip->base) > (128 << 20))
	...
	for (i = 0; i < addresslen; i++)
		printk("ADDR[%d]= 0x%x. ", i, zed_address[i]);
	printk("\n");
#endif
...
static void zed_nand_read_buf(struct nand_chip *chip, uint8_t *buf, int len)
{
	int i;
	struct mtd_info *mtd = nand_to_mtd(chip);
	//register struct nand_chip *chip = mtd->priv;
...
static void zed_nand_write_buf(struct nand_chip *chip, const uint8_t *buf, int len)
{
	int i;
	struct mtd_info *mtd = nand_to_mtd(chip);
	//register struct nand_chip *chip = mtd->priv;
...
{
	struct zed_nand_controller *host = dev_id;
...
static int zed_nand_chip_check_interface(struct zed_nand_controller *host,
        int mode)
{
	...
	ret = host->chip.legacy.get_features(&host->chip,
...
#if 0
static int zed_nand_chip_init_timings(struct zed_nand_controller *host,
                                      int mode)
{
	int ret = 0;
	int set_mode;
	...
	ret = host->chip.legacy.set_features(&host->chip,
	                                   ONFI_FEATURE_ADDR_TIMING_MODE, feature);
	udelay(10);
	return ret;
}
...
static struct zed_nand_controller *zed_nand_host;
...
static int zed_nand_probe(struct platform_device *pdev)
{
...
#endif
	zed_nand_host = devm_kzalloc(&pdev->dev, sizeof(*zed_nand_host), GFP_KERNEL);
	if (!zed_nand_host) {
		dev_err(&pdev->dev, "failed to allocate device structure.\n");
		return -ENOMEM;
	}
	nand_controller_init(&zed_nand_host->controller);
	zed_nand_host->pdev = pdev;
	
	nand_chip = &zed_nand_host->chip;
	nand_chip->controller = &zed_nand_host->controller;
	...
	nand_set_controller_data(nand_chip, zed_nand_host);
	...
	zed_nand_host->dma_io_phys = (dma_addr_t)dma_reg->start;
	zed_nand_host->misc_regs = devm_ioremap_resource(&pdev->dev, misc_reg);
	zed_nand_host->sdr_reg = devm_ioremap_resource(&pdev->dev, sdr_reg);
	zed_nand_host->nvddr_reg = devm_ioremap_resource(&pdev->dev, nvddr_reg);
	zed_nand_host->dma_regs = devm_ioremap_resource(&pdev->dev, dma_reg);


#ifdef ENABLE_DEBUG
	printk("remap misc_reg is start 0x%x.\n", (int)zed_nand_host->misc_regs);
	printk("remap sdr_reg is start 0x%x.\n", (int)zed_nand_host->sdr_reg);
	printk("remap nvddr_reg is start 0x%x.\n", (int)zed_nand_host->nvddr_reg);
	printk("remap dma_regs is start 0x%x.\n", (int)zed_nand_host->dma_regs);
#endif
	if (IS_ERR(zed_nand_host->misc_regs) || IS_ERR(zed_nand_host->sdr_reg) || IS_ERR(zed_nand_host->nvddr_reg)) {
		err = -EIO;
	...
	if (prop) {
		if (be32_to_cpup(prop) == 0) {
			zed_nand_host->mode = SDR_NAND_MODE;
		...
		dev_info(&pdev->dev, "zed,nand-mode doesn't exist in dts, using defualt SDR mode.\n");
		zed_nand_host->mode = SDR_NAND_MODE;
	}

	zed_nand_host->has_dma = of_property_read_bool(pdev->dev.of_node, "zed,nand-has-dma");
	zed_nand_host->irq = platform_get_irq(pdev, 0);
	dev_info(&pdev->dev, "Get NAND pl interrupter num is [%d] \n", zed_nand_host->irq);

	if (zed_nand_host->irq < 0) {
		retval = -ENXIO;
	...
	init_completion(&zed_nand_host->comp);

	nand_chip->legacy.IO_ADDR_R = zed_nand_host->misc_regs;
	nand_chip->legacy.IO_ADDR_W = zed_nand_host->misc_regs;
//	nand_chip->legacy.chip_delay = 50;
	nand_chip->legacy.cmd_ctrl = __cmd_ctrl;
	nand_chip->legacy.cmdfunc = __nand_command;
	/* nand_chip->dev_ready = getDeviceReadyPin; */
	nand_chip->legacy.read_buf = zed_nand_read_buf;
	nand_chip->legacy.write_buf = zed_nand_write_buf;
	nand_chip->legacy.read_byte = zed_nand_read_byte;
	nand_chip->bbt_options |= NAND_BBT_USE_FLASH;
	nand_chip->options |= NAND_NO_SUBPAGE_WRITE;

	platform_set_drvdata(pdev, zed_nand_host);
	fpga_init(mtd);

	nand_chip->ecc.mode = NAND_ECC_SOFT;
	nand_chip->ecc.algo = NAND_ECC_BCH;  
	retval = nand_scan(nand_chip, 1);
	if (retval)
		goto out_free_data; 
	
	NS_INFO("Using %u-bit/%u bytes BCH ECC\n", nand_chip->base.eccreq.strength,
	        nand_chip->base.eccreq.step_size);
	//nand_chip->ecc.mode = NAND_ECC_SOFT;
	//nand_chip->ecc.algo = NAND_ECC_BCD;
	nand_chip->ecc.size = nand_chip->base.eccreq.step_size;
	/*
	...
	*/
	...
	while (1) {
		...
		if (((nand_chip->ecc.size * 8) >> m) == 0) {
			nand_chip->ecc.bytes = (nand_chip->base.eccreq.strength * m + 7) / 8;
			pr_warn("strength_ds = %d, bytes = %d\n",
			        nand_chip->base.eccreq.strength, nand_chip->ecc.bytes);
			break;
		}
	}
	nand_chip->ecc.strength = nand_chip->ecc.bytes * 8 / fls(8 * nand_chip->ecc.size);//bean add for 4.0

	NS_INFO("nand_chip->ecc.size %d, nand_chip->ecc.strength %d  \n", nand_chip->ecc.size, nand_chip->ecc.strength);

	if ((ENABLE_DMA == 1) && (zed_nand_host->has_dma)) {
		DMAInit(mtd);
		use_dma = 1;
	}
	if (use_dma)
	...
	if (mode_transfer != zed_nand_host->mode) {

		retval = zed_nand_chip_init_timings(zed_nand_host, mode_transfer);
		if (!retval) {

			zed_nand_host->mode = mode_transfer;
			NS_INFO("Send transfer-mode to %s command OK.\n",
			        (zed_nand_host->mode == DDR_NAND_MODE) ? "DDR" : "DDR2");

			change_host_mode(zed_nand_host, DDR_NAND_MODE);
			/*nand_chip->options |= NAND_OWN_BUFFERS */
		}
		retval = zed_nand_chip_check_interface(zed_nand_host, mode_transfer);
		...
		if (retval) {
			...
			change_host_mode(zed_nand_host, SDR_NAND_MODE);
			zed_nand_host->mode = SDR_NAND_MODE;
		} else
	...
	if (ENABLE_INTERRUPTER == 1) {
		retval = devm_request_irq(&pdev->dev, zed_nand_host->irq, zynq_irq_handler,
		                          0, pdev->name, zed_nand_host);
		if (retval != 0) {
			retval = -ENXIO;
	...
	if (retval)
		goto cleanup_nand;
#if 0
	err = mtd_device_parse_register(&zed_nand_host->mtd, NULL, &ppdata, NULL, 0);
	if (!err) {
		powerloss_cdev_init();
...
#endif
	dev_info(&pdev->dev, "Zed-zynq PL NAND Controller registered successfully.\n");
	return 0;

cleanup_nand:
	nand_cleanup(nand_chip);
out_free_data:
	if (zed_nand_host->dma_chan)
		dma_release_channel(zed_nand_host->dma_chan);
	kfree(zed_nand_host);
	dev_info(&pdev->dev, "Zed-zynq PL NAND Controller registration failed.\n");
	return retval;
}
...
static int zed_nand_remove(struct platform_device *pdev)
{
	...
	struct zed_nand_controller *nfc = platform_get_drvdata(pdev);
	...
	powerloss_cdev_exit();
	kfree(nfc);
	dev_info(&pdev->dev, "Zed-zynq PL NAND Controller de-registered successfully.\n");
	return 0;
}
